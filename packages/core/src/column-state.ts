...
export function applyColumnState<TRow>(
  ...
  columnStates: readonly ColumnState[] = [],
...
export function setColumnPinned(
  state: GridState,
  columnId: string,
  pinned: ColumnState["pinned"] | undefined,
): GridState {
  return updateColumnState(state, columnId, { pinned });
}

export function setColumnOrder(state: GridState, columnIds: readonly string[]): GridState {
...
function updateColumnState(
  state: GridState,
  columnId: string,
  patch: Omit<Partial<ColumnState>, "columnId">,
): GridState {
